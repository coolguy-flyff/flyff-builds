...
describe('Seraph healing rows (plan B7.3)', () => {
  ...
  it('pins STA 400 / INT 200 without healing bonus', () => {
    expect(computeHealingSkills(data, healerContext(stats, 0), DEFAULT_ENGINE_OPTIONS)).toEqual({
      healRain: 2254,
      gloriaPatri: 6408,
      gloriaPatriEffectIncrease: 6740,
    });
  });

  it('scales by the healing rate after the synergy', () => {
    const healed = computeHealingSkills(data, healerContext(stats, 20), DEFAULT_ENGINE_OPTIONS);

    expect(healed.gloriaPatri).toBe(7689);
    expect(healed.gloriaPatriEffectIncrease).toBe(8088);
    expect(healed.healRain).toBe(2704);
  });
  ...
  it('matches Flyffulator when the Heal synergy is switched off', () => {
    ...
    expect(healed.gloriaPatri).toBe(5408);
    expect(healed.gloriaPatriEffectIncrease).toBe(5740);
    expect(healed.healRain).toBe(2254);
  });
});
